...
export async function runIntentRecognitionAgent(input: {
  ...
  const systemPrompt = [
    ...
    'intent 只允许：general_chat, knowledge_qa, analysis, creation, advice, write_blog, publish_post, update_post, delete_post, get_post_detail, list_drafts, create_thought, create_bookmark, list_bookmarks, search_thoughts, answer_thoughts。',
...
export async function runBusinessToolAgent(input: {
  ...
  return invokeJsonAgent<ChatToolCall>({
    systemPrompt,
    userPrompt,
    fallback,
